...
modifying `Function.prototype`.  Note that you can also pass additional arguments to the returned/new function during invocation.
...
* `qq.android()` - Returns true of the current user agent is running on an Android device.

* `qq.ios()` - Returns true if the current device is an iPad, iPhone, or iPod.
